  ...
  const totalKits = rabbits.reduce(
    (sum, rabbit) =>
      ...
      (rabbit?.birth_history?.reduce(
        (kitSum: any, history: any) => kitSum + (history.number_of_kits || 0),
        0
      ) || 0),
    0
  );
            ...
            <thead>
              <tr className="border-b border-gray-100 dark:border-gray-800 bg-gray-50/50 dark:bg-gray-800/50">
                <th className="text-left p-4 w-12">
                  <Checkbox
                    checked={
                      selectedRabbits.length === filteredRabbits.length &&
                      filteredRabbits.length > 0
                    }
                    onCheckedChange={(checked) =>
                      setSelectedRabbits(
                        checked
                          ? filteredRabbits?.map((r: any) => r.rabbit_id)
                          : []
                      )
                    }
              ...
              <AnimatePresence>
                {filteredRabbits.map((rabbit, index) => (
                  ...
                  >
                    <td className="p-4">
                      <Checkbox
                        checked={selectedRabbits.includes(
                          rabbit.rabbit_id ?? ""
                        )}
                        onCheckedChange={() =>
                          handleCheckboxChange(rabbit.rabbit_id ?? "")
                        }
                        className="rounded-md"
                      />
                    </td>
