...
export default function AddKitDialog({ rabbit, doeId, buckId, doeName, buckName, onClose, onKitAdded }: AddKitDialogProps) {
    ...
    useEffect(() => {
        ...
        const fetchKits = async () => {
            try {
                const response = await axios.get(`${utils.apiUrl}/breeds/kits/${user.farm_id}`, {
                ...
                const maxNumber = existingKits.reduce((max: number, kit: any) => {
                    const match = kit.kit_number.match(/^RB-(\d+)$/);
        ...
        for (const kit of kits) {
            if (!kit.kit_number || !kit.status) {
                toast({
                    variant: "destructive",
                    title: "Invalid data",
                    description: `Kit number and status are required for kit ${kit.kit_number}.`,
                });
                return false;
            }
            if (kit.birth_weight && (isNaN(parseFloat(kit.birth_weight)) || parseFloat(kit.birth_weight) <= 0)) {
                toast({
                    variant: "destructive",
    ...
    const handleSubmit = async (e: React.FormEvent) => {
        ...
        try {
            ...
            if (!specificBreedingRecord) {
                ...
                const breedingResponse = await axios.post(
                    `${utils.apiUrl}/breeds/${user.farm_id}`,
                    {
                        doe_id: doeId,
                        buck_id: buckId || null,
                        mating_date: new Date(new Date(actualBirthDate).getTime() - 30 * 24 * 60 * 60 * 1000).toISOString().split("T")[0], // Assume mating 30 days before birth
                ...
                breedingRecordId = breedingResponse.data.data.id;
            } else {
            ...
            const kitData = kits.map((kit) => ({
                ...
                birth_weight: kit.birth_weight ? parseFloat(kit.birth_weight) : null,
                gender: kit.gender || null,
                color: kit.color || null,
                status: kit.status || null,
                parent_male_id: buckId || null,
